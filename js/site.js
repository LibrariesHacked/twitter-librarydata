$(function () {
  ...
  twitterlibraries.loadLists(function () {
    ...
    var ctx_bar_hashtags = document
      .getElementById('cht_hashtags')
      .getContext('2d')
        ...
        onClick: function (e, activeElements) {
          window.open(
            'https://twitter.com/hashtag/' +
              bar_hashtags.data.labels[activeElements[0]._index],
            '_blank'
          )
        }
      }
    ...
    var ctx_bar_mentions = document
      .getElementById('cht_mentions')
      .getContext('2d')
    var bar_mentions = new Chart(ctx_bar_mentions, {
      ...
      options: {
        ...
        onClick: function (e, activeElements) {
          window.open(
            'https://twitter.com/' +
              bar_mentions.data.labels[activeElements[0]._index],
            '_blank'
          )
        }
      }
    ...
    var setHashtags = function (list) {
      ...
      for (var i = 0; i < twitterlibraries.hashtags[list].length; i++) {
        if (!hashtags[twitterlibraries.hashtags[list][i]])
          hashtags[twitterlibraries.hashtags[list][i]] = 0
        hashtags[twitterlibraries.hashtags[list][i]] =
          hashtags[twitterlibraries.hashtags[list][i]] + 1
      }
      var top_five = Object.keys(hashtags)
        .sort(function (a, b) {
    ...
    var setMentions = function (list) {
      ...
      for (var y = 0; y < twitterlibraries.mentions[list].length; y++) {
        if (!mentions[twitterlibraries.mentions[list][y]])
          mentions[twitterlibraries.mentions[list][y]] = 0
        mentions[twitterlibraries.mentions[list][y]] =
          mentions[twitterlibraries.mentions[list][y]] + 1
      }
      var top_five = Object.keys(mentions)
        .sort(function (a, b) {
          return mentions[b] - mentions[a]
        })
        .slice(0, 5)
    ...
    var setJoined = function (id) {
      ...
      line_joined.data.labels = $.map(
        Object.keys(twitterlibraries.month_counts[id]),
        function (d, i) {
      ...
      line_joined.data.datasets.push({
        ...
        borderWidth: 1,
        label: '',
        data: Object.keys(twitterlibraries.month_counts[id]).map(function (
          mth,
          y
        ) {
